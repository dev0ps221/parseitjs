...
class ParseIt{
    ...
    merge_nodes(node,previous_node,next_node,current_pos,previous_pos,next_pos,raw_ast)
    {
    ...
    tokenize_input(args=[],use_input=null)
    {
        const clean_token   =   [] 
        if(args.length)
        {
    ...
    parse(args=[],use_input)
    {
        if(!use_input)
        {
            use_input = this.tokenize_input(args,use_input)
        }
